...
# Telegram function to get solana price every 5 minutes
# how to make it work in a loop
async def solana_daily_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    while True:
        crypto = "solana"
        data = get_crypto_data(crypto)
        if data:
...
app.add_handler(CommandHandler("Daily_solana", solana_daily_update)) #TSGD
